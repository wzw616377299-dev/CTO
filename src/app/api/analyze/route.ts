...
- **时序图 (sequenceDiagram)**：适合展示系统交互、API 调用、多方协作
- **思维导图 (mindmap)**：适合展示概念层级、知识结构
- **泳道图**：适合展示多角色协作流程（使用 flowchart 的 subgraph 实现）

图表要求：
1. 节点文字简洁，不超过10个字
2. 流程清晰，分支不超过3层
3. 使用 \`\`\`mermaid 代码块包裹
4. **重要**：subgraph 和节点名称只能使用英文、数字、下划线，不要使用中文括号、空格等特殊字符
5. 如果需要中文标签，使用引号包裹：\`subgraph "用户端"\`
...
根据内容类型自动选择合适的图表（使用 Mermaid 语法）：
- **架构图 (flowchart)**：适合展示系统架构、模块关系
- **流程图 (flowchart)**：适合展示数据处理流程、业务流程
- **时序图 (sequenceDiagram)**：适合展示服务调用、数据流转
...
- **流程图 (flowchart)**：适合展示概念的工作原理、执行流程

图表要求：
1. 节点文字简洁，不超过10个字
2. 层级清晰，展示概念的关系
3. 使用 \`\`\`mermaid 代码块包裹
4. **重要**：节点名称只能使用英文、数字、下划线，不要使用中文括号、空格等特殊字符
5. 如果需要中文标签，使用引号包裹
